...
function createElementAttribute() {
    return {__html: `type="button"`};
}

const todos = [{time: "day 1", content: "eat"}, {time: "day 2", content: "sleep"}]
    ...
    <Container id="calendar">
        ...
        <ul>{
            todos.map((todo) => (
                <li>
                    <div>
                        <input class={["btn btn-primary"]} type={"button"} data-bs-toggle={"popover"} data-bs-trigger={"focus"} title={todo.time} data-bs-content={"Event: 0"} value={"Event: 0"} data-bs-original-title={"1:00AM"} aria-label={"1:00AM"}/>
                    </div>
                </li>
            ))
        }</ul>
